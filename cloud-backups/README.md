...
- The `DROP` is the sole irreversible step and runs **only after a verified upload**. By default, verification is a **byte-integrity** gate: the upload completed (on real AWS, S3 verifies a SHA-256 of every part server-side and refuses on mismatch) **+** a stored-size match **+** a whole-object SHA-256 written as a `<key>.sha256` sidecar. This does **not** by itself prove the archive is *restorable*.
- **Bucket permissions on S3: the default path needs write + list, NOT content read.** Existence/size lookups go through `ListObjectsV2`, so the credentials need:
  - `s3:PutObject` on the objects: `arn:aws:s3:::<bucket>/<dump-prefix>-*`
  - `s3:ListBucket` on the **bucket**: `arn:aws:s3:::<bucket>` -- a bucket-level ARN with *no* `/*`, which is the usual reason a hand-written policy silently fails to grant it
  - `s3:AbortMultipartUpload` on the objects -- a multi-GB dump goes multipart and an interrupted upload aborts it; without this the abort 403s and leaves billed orphan parts (alternatively, add an "abort incomplete multipart uploads" lifecycle rule)
  - `s3:GetObject` **only** for `--verify-restore` (plus `kms:Decrypt` on an SSE-KMS bucket). `s3:DeleteObject` is never needed -- the tool never deletes.

  If you scope `s3:ListBucket` with an `s3:prefix` condition, it must admit the **full object key**, e.g. `StringLike: {"s3:prefix": "<dump-prefix>-*"}`. A folder-style condition (`"my-audit/"`) or any `s3:delimiter` condition will deny the request, because the lookup passes the whole key as the prefix and sends no delimiter -- and a denial is deliberately not read as "absent", so the run quarantines instead of reclaiming disk. Grant `s3:ListBucket` even on a Head-based flow: without it S3 answers 403 rather than 404 for a *missing* key, which cannot be treated as a definitive absence, so the self-healing re-dump path stalls. (On Azure the equivalent lookup is `GetProperties`, which still requires blob **read**.)
